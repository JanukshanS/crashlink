...
| FR-DEMO-01 Seed: admin, Nimal, Ravi, contact, Scooter 1 ↔ CL-0001 | PASS (fixed) | A:demo.test.ts `FR-DEMO-01 seed › creates the admin, Nimal, Ravi, the contact, the judge and Scooter 1 paired to CL-0001`; `› keeps device secrets on a re-seed, so the real bike is not locked out` |
